...
func TestProposeFromFailures(t *testing.T) {
	...
	runs := []brain.EvalRun{
		{RunID: "r1", Question: "partner GMV for pine-hollow", AggregateVerdict: "fail"},
		{RunID: "r2", Question: "passing one", AggregateVerdict: "pass"},
		{RunID: "r3", Question: "partner GMV for pine-hollow", AggregateVerdict: "fail"}, // dup question
	}
	...
	if !strings.Contains(out, "partner GMV for pine-hollow") {
	...
	if strings.Count(out, "partner GMV for pine-hollow") != 1 {
