...
// a question about the ButterStack org, and may confidently declare
// "none of these are butterstack" based on an off-scope result set.
...
- If a "PRIOR TURN" block is provided above the question, the user may be asking a follow-up that REFERS to that prior turn ("do the same thing, but for issues", "again for last month", "what about in butter_stack", "and for Y", "also X").
- When the current question uses such a reference AND the prior turn is present: (1) INHERIT raw_entities from the prior turn except where the current question explicitly changes them; (2) ALSO set effective_question to a standalone rewrite that weaves the prior turn's scope into the current question so downstream stages do not need to know about the prior turn. Example: prior "what are my open PRs across butterstack and aida* github repos" + current "do the same thing, but for issues" → effective_question: "what are my open issues across butterstack and aida* github repos". The action/keywords follow the current question; timeframe is extracted from the current question only (not inherited).
...
- For grep/codebase/docs sources: return a regex search pattern to grep for - NOT a shell command, NOT a gh command, NOT a JSON blob. Example valid patterns: 'class AuthHandler', 'camp-butz.*(live|test)', 'def handle_webhook', 'ValueError.*camp-butz'. Example INVALID outputs: 'search issues --owner ...', 'gh pr list --repo ...'. If the user asks about "issues" or "PRs" involving a codebase, still emit a grep pattern of the symptom/code they are asking about, not a gh command - the planner routes gh separately.
...
- For GitHub (gh) sources: the exec template is 'gh {query}' - return ONLY the arguments after 'gh'. Choose the RIGHT subcommand for the scope:
  ...
  Source YAML fields like 'repo: owner/name' are AUTHORITATIVE - copy that value verbatim into --repo. Do not rename the owner or add your own. If the question spans multiple specific repos (e.g. "butterstack AND aida"), emit 'search prs --repo owner1/repo1 --repo owner2/repo2 --state open --json repository,number,title,state,url' - 'search prs' accepts multiple --repo filters. If the user asks about a whole org, use --owner <org>.
