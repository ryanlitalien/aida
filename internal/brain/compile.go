...
func (b *Brain) Index(ctx context.Context) error {
	...
	// 4. Index knowledge-domain pages (generated source profiles and
	// hand-written prose pages alike) into knowledge_pages + corpus_fts so
	// brain_search can find them. See knowledge_index.go.
	domainCount := 0
	if res, err := b.IndexKnowledgeDomains(ctx); err != nil {
		ui.PrintVerbose("Brain index", "knowledge domains: "+err.Error())
	} else {
		domainCount = res.Indexed
	}
